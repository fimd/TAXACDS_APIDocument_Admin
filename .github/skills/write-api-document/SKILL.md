---
name: write-taxacds-api-document
description: "Dùng khi tạo mới hoặc cập nhật tài liệu HTML cho API TAXACDS. Trigger keywords: viết tài liệu API TAXACDS, viết tài liệu API TaxCDS, tạo tài liệu HTML API TAXACDS, html api doc taxacds, document api taxacds, cập nhật tài liệu API TAXACDS."
---

# Write TAXACDS API Document

## Ghi chú quan trọng

API cho TAXACDS thì dùng folder `TAXACDS_APIDocument_Partner`.

---

## Khi nào dùng skill này

- Tạo mới trang HTML tài liệu cho một hoặc nhiều API endpoint TAXACDS
...
**File đặt tên theo quy tắc:** `{ModuleName}_{ApiName}.html` — ví dụ: `TaxCDSMstLookup_LookupMst.html`
...
| 3 | **Headers schema** | Bảng 3 cột: Header \| Required \| Mô tả. Với TAXACDS thì đọc theo route thực tế |
| 4 | **Body schema** | Bảng 5 cột: Field \| Type \| Required \| Rule \| Mô tả. Mỗi field một row, kể cả nested dạng `filter.taxCode` |
...
| 8 | **Tham khảo** | Luôn có link `Common.html#common-error`; nếu API có status/common constant khác thì thêm link tương ứng |
...
| **429** | Auth pre-function fail (apiKey/apikey sai / hết quota) | `QUOTA_EXCEEDED` |
...
Chỉ dùng test key/clientId/apikey do user cung cấp. Không bao giờ dùng key production.
...
- [ ] Đúng folder `document/TAXACDS_APIDocument_Partner/`
...
- [ ] Card Headers schema: tách riêng, đúng header thực tế theo route
...
- [ ] Card Tham khảo: link `Common.html#common-error` hoặc relative path đúng theo vị trí file
...
- **Ví dụ chuẩn**: `document/TAXACDS_APIDocument_Partner/modules/TaxCDS/TaxCDSMstLookup_LookupMst.html`
